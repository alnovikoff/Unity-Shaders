...
fragmentInput vertexProgram(vertexInput data)
{
    ...
    float offset = tex2Dlod(_HeightMap, float4(data.uv.xy, 0, 0)).r * _HeightAmount;
    data.positionOS.y += offset;
...
{
    float height = tex2D(_HeightMap, input.uv).r;
    float4 color = SAMPLE_TEXTURE2D(_BaseColor, sampler_BaseColor, input.uv_MainTex) * lerp(_BottomColor, _Color, height);
