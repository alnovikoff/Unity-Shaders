...
fragmentInput vertexProgram(vertexInput data)
{
    ...
    output.uv = TRANSFORM_TEX(data.uv, _BaseColor);
    output.uv += 0.5 * float2(_Time.y, 0) * _WaveSpeed;

    //output.uv_wave = data.uv;
    //output.uv_wave = output.uv_wave * _Value1;

    /*output.uv_wave.x = sin(((output.uv_wave.x * _Value2)) + sin(-output.uv_wave.x * _Value2)) + _Time.y * _Speed;
    float3 vert = data.positionOS;
    vert.y = output.uv_wave.x * _Value3;
    float3 pos = TransformObjectToWorld(vert);
    output.positionCS = TransformWorldToHClip(pos);

    output.uv_wave.y = sin((output.uv_wave.y * _Value6) + _Time.y * _Speed);
    float3 vert2 = data.positionOS;
    vert2.y = output.uv_wave.y * _Value7;
    float3 pos2 = TransformObjectToWorld(vert2);
    output.positionCS = TransformWorldToHClip(pos2);*/

    float3 p = data.positionOS.xyz;
    float k = 2 * 3.14 / _WaveLenth;
    ...
    float3 tang = normalize(float3(1 - k * _Steepness * sin(f), k * _Steepness * cos(f), 0));
    float3 normal = float3(-tang.y, tang.x, 0);
    data.positionOS.xyz = p;
    data.normalOS = normal;
    ...
    float4 vert3 = data.positionOS;
    output.uv.y += cos(_WaveSpeed * windSample) * _WaveAmp;
    output.uv.x += sin(_WaveSpeed * windSample) * _WaveAmp;
    float3 pos3 = TransformObjectToWorld(vert3);
